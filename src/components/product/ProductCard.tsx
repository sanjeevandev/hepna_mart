...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <Link to={`/product/${product.slug}`} className="card card-hover flex flex-col group relative overflow-hidden transition-all duration-300">
      <div className="relative aspect-square overflow-hidden bg-surface flex items-center justify-center">
        ...
        >
          <Heart className={`w-5 h-5 ${wishlisted ? 'fill-accent text-accent' : 'text-gray-400'}`} />
        </button>
      </div>

      <div className="p-4 flex flex-col flex-grow">
        <span className="text-xs text-gray-500 uppercase tracking-wider mb-1 font-medium">{product.brand}</span>
        <h3 className="font-semibold text-primary-dark mb-2 line-clamp-2 min-h-[3rem] text-sm md:text-base">{product.name}</h3>
        ...
        <div className="flex items-center gap-2 mt-auto" onClick={(e) => e.preventDefault()}>
          ...
          <Button 
            variant="primary" 
            className="flex-grow text-sm py-1.5 px-2"
